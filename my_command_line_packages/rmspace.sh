...
# Language		:	GNU bash, version 4.3.48
# Concise		: 	Searches for contents (files, directories) which contain given PATTERN in their name
#					and substitutes the PATTERN with a given STRING


function usage() {
	echo "Usage: $( basename $0 ) -f|d|a [-rvi] [-p PATH] [-m M] [--replace PAT] [--replacewith STR]"

	echo $'\nSearches for contents which contain PAT in their name and substitute PAT with STR.'

	echo "
options     long options                 Meaning
 -f         --files                 Performs on files.
 -d         --directories           Performs on directories.
 -a         --all                   Performs on all contents (files and
                                      directories) including the contents
                                      named with leading dot.
 -r         --recursive             Performs on sub-directories too recursively.
 -v         --verbose               Explain what is being done.
 -i         --interactive           Enable interactive mood to prompt before
                                      renaming.
 -p PATH    --path PATH             PATH is the directory to start travarsing
                                      and renaming. Default is present working
                                      directory.
 -m M       --maxdepth M            Descend at most M (a non-negative integer)
                                      levels of directories below the
                                      starting-points. If recursive mood is not
                                      enabled, by Default M is 1 which means
                                      the contents of startin path.
            --replace PAT           The pattern PAT (string or regex) is to
                                      find in content names to be replaced.
                                      Default is a blank space.
            --replacewith STR       The string STR will substitute PAT.
...
Exit status:
    ...
    1        'getopt --test' failed in this environment, GNU enhanced getopt is not found.
    2        getopt has complained about wrong arguments.
    3        Unrecognized options encountered in command.
"
}
...
OPTIONS=rp:m:vidfa
LONGOPTIONS=recursive,path:,maxdepth:,verbose,interactive,directories,files,all,replace:,replacewith:,help
...
if ( ! "$opt_files" ) && ( ! "$opt_directories" ) && ( ! "$opt_all" ) ; then
	usage
	exit 0
fi
